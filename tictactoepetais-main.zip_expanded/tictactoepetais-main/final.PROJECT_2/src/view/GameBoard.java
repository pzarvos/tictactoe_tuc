...
public class GameBoard extends GamePanel {	
	...
	public void drawGrid(Graphics g) {
		...
		for (int i=1; i<3; i++) {
			//Draw Vertical Line
			g2d.drawLine(
					boardZero().x +i*cellSize(),boardZero().y, 				
					boardZero().x+i*cellSize(),boardZero().y+boardSize() 	
					);
			
			//Draw Horizontal Line
			g2d.drawLine(
					boardZero().x,  boardZero().y+i*cellSize(), 			
					boardZero().x+boardSize(), boardZero().y+i*cellSize()	
					);
		}
